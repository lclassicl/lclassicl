				...
				<fieldset id="bo_sch">
					...
					<form name="fsearch" method="get" action="/forum/board/${boa.boa_url}/search">
						...
						<select name="searchCondition" id="sfl" class="frm_input" size="1">
							<option value="art_title" <c:if test="${'art_title' == searchCondition }">selected</c:if>>제목</option>
							<option value="art_content" <c:if test="${'art_content' == searchCondition }">selected</c:if>>내용</option>
							<option value="user_nick" <c:if test="${'user_nick' == searchCondition }">selected</c:if>>글쓴이</option>
						</select> 
						<label for="stx" class="sound_only">검색어 
							<strong class="sound_only"> 필수</strong>
						</label> 
						<input type="text" name="searchKeyword" value="${searchKeyword}" required id="stx" class="frm_input required" size="15" maxlength="20"> 
